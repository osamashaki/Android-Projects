...
echo json_encode($response);
	...
	$email = $_POST['email'];
		...
		if ($db->isUserExisted($email)) 
		{
			...
			$result = mysql_query("UPDATE student SET encrypted_password = '$encrypted_password',salt = '$salt' 
						  WHERE email = '$email' ");
...
else if ($tag == 'saveresult')
{
	...
	$data = mysql_fetch_array(mysql_query("SELECT uid FROM student WHERE firstname='$name' "));
	$sid = $data[0];

	$testdate = date("d/m/Y");
		
			$result = mysql_query("insert into result values('', '$sid','$a','$b','$c','$testdate') ");
